...
public class Grid {
	...
	public Grid(CMS_GENERIC_Lib aut, String gridXpath) {
		this.aut = aut;
		driver = aut.getBrowser().getWebDriver();
		this.XPath = gridXpath;
	}

	public static Grid getGrid(CMS_GENERIC_Lib aut, String sectionName) {
		String xpath = "//table[@class='taglib-search-iterator']";
		System.out.println("Get Table On Page: " + sectionName);
		return new Grid(aut, xpath);
	}

	public Grid(CMS_GENERIC_Lib aut, String gridXpath, int columnHeaderIndex, int firstRowIndex) {
		// this.aut = aut;
		// driver = aut.getBrowser().getWebDriver();
		// this.XPath = gridXpath;
		this(aut, gridXpath);
	...
	public void readColumnHeaders() {
		if (columnHeaders == null) {
			...
			String tdXPath = this.XPath + "/tbody/tr[" + columnHeaderIndex + "]/th";
	...
	public void assertColumnHeaderExists(String columnHeaderName) throws Exception {

		try {
			org.testng.Assert.assertTrue(columnHeaderExists(columnHeaderName), columnHeaderName + " does not exist");
	...
	public List<WebElement> getDataRowsInCurrentPage() {
		String dataRowXPath = this.XPath + "/tbody/tr[td/a or ' ']";
		// + "/tbody/tr[td[starts-with(@class,'tableDataUnderscore')]]";
	...
	public WebElement findDataRow(int columnIndex, String cellText) {
		...
		for (int i = 0; i < dataRows.size(); i++) {
			...
			// String cellXPath =
			// "//td[starts-with(@class,'tableDataUnderscore')]["+(columnIndex+1)+"]";
